...
```{r}
depth_and_rock_info <- read_excel(file.path("metadata", "aromaticSRM_20170103.xlsx")) %>% 
  rename(rock.g = `rock`) %>% 
...
final_data_iso <- subset(final_data, conc_rock.ug_g<1)
View(final_data_iso)
...
subset(final_data_iso, peak_name=='Isorenieretane') %>%
  ggplot() +
  aes(x = conc_rock.ug_g, y = depth) +
  geom_point() +
  #facet_wrap(~peak_name, scales = "free") 
...
subset(final_data_iso, peak_name=='Chlorobactane') %>%
  ggplot() +
  aes(x = conc_rock.ug_g, y = depth) +
  geom_point() +
  #facet_wrap(~peak_name, scales = "free") 
...
subset(final_data_pah, peak_name=='all_Aryl_Isop') %>%
  ggplot() +
  aes(x = conc_rock.ug_g, y = depth) +
  geom_point() +
  #facet_wrap(~peak_name, scales = "free") 
...
subset(final_data_iso, peak_name=='all_PAH') %>%
