...
final_data <- final_data1 %>% 
    group_by(file_id) %>% 
        do({
          bind_rows(., 
              ratio_peaks(., "5ring_all", "2ring_all", "5/2 ring")
 ) }) %>% ungroup() 
...
```{r}
write.csv(final_data, file = "Aromatic_SRM_SH1")
```
...
```{r}
aliphatic <- read.csv(file.path("metadata","aliphatic.csv")) %>% select("file_id", "peak_name", "area.cpm", "depth", "amount.ug", "conc_rock.ug_g",  "ratio")

final_data <- final_data %>% select("file_id", "peak_name", "area.cpm", "depth", "amount.ug", "conc_rock.ug_g",  "ratio")

final_data <- union(final_data, aliphatic, by = "depth") 
```
...
```{r "Carotenoids_against_depth", fig.width = 15, fig.height = 5}
...
###all PAH combos
```{r "PAHs_against_depth", fig.width = 15, fig.height = 5}
...
###Individuals
```{r "2ringPAH_indiv", fig.width = 5, fig.height = 5}
...
```{r "3ringPAH_indiv", fig.width = 15, fig.height = 8}
...
###4 ring PAHs
```{r "4ringPAH_against_depth", fig.width = 15, fig.height = 5}
...
###5 ring PAHs
```{r "5ringPAH_against_depth", fig.width = 15, fig.height = 5}
...
###6 ring PAHs
```{r "6ringPAH_against_depth", fig.width = 5, fig.height = 5}
...
###ratio PAHs
```{r "ratioPAH_against_depth", fig.width = 5, fig.height = 5}

ratio <- subset(final_data, peak_name %in% c("5/2 ratio")) %>%
  ggplot() +
  aes(x = depth, y = ratio) +
  geom_point() +
  geom_line() +
...
###all individual PAHs
```{r "allindPAH_against_depth", fig.width = 25, fig.height = 8}
